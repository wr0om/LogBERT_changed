...
def compute_anomaly(results, params, seq_threshold=0.5):
    ...
    return the number of anomaly sequences, an anomaly sequence is defined as a sequence with
     over half (or whatever threshold) of masked tokens are undetected, or is far away from the center (above the radius)
    """
...
def find_best_threshold(test_normal_results, test_abnormal_results, params, th_range, seq_range, path):
    ...
    seq_range = seq_range[:len(recall_arr)]
    plt.plot(seq_range, recall_arr, label="recall")
    plt.plot(seq_range, precision_arr, label="precision")
    plt.plot(seq_range, F1_arr, label="F1")
...
class Predictor():
    ...
    def detect_logkey_anomaly(self, masked_output, masked_label):
        """
        TODO: CHANGE TO IMPLEMENT SINGLE ACTIVITY ANOMALIES
    ...
    def helper(self, model, output_dir, file_name, vocab, scale=None, error_dict=None):
        ...
        # TODO: CHANGE IN LogDataset THE MASKING TO MASK 1 ACTIVITY PER SESSION PER TIME (IDK YET)
